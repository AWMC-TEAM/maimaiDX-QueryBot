...
class UpdateTable:
    ...
    async def update_level_15_rating_table(self) -> None:
        ...
        fot.draw(700, height - 30, 30, f'Generated by {maiconfig.botName} BOT', assets.font_color, 'mm')
        ...
        for i in range(lines * 3):
            ...
            if i < count:
                song = lv15[i]
                chart = Image.open(music_picture(song.id)).convert('RGBA')
                im.alpha_composite(chart.resize((330, 330)), (x + 10, y + 10))
                im.alpha_composite(assets.table_type_bg[song.type], (x + 200, y + 345))
                ver_img = pic(f'{song.basic_info.version}.png')
                if ver_img.exists():
                    im.alpha_composite(Image.open(ver_img).resize((332, 160)), (x + 9, y - 80))
                fot.draw(x + 100, y + 370, 35, song.id, assets.font_color, 'mm')
            else:
    ...
    def _draw_plate(
        ...
        remaster_song_set = set(remaster_songs or [])
        start_y = 490
        for ds, songs in level_dict.items():
            ...
            def sort_key(m: Music) -> float:
                if remaster_id_list is not None and int(m.id) in remaster_set and m in remaster_song_set:
            ...
            for num, music in enumerate(songs):
                ...
                im.alpha_composite(cover, (x, y))
                is_remaster = remaster_songs is not None and music in remaster_song_set
