...
from .maimaidx_storage import backend_name, sync_configured_backend
...
async def sync_storage_now() -> Optional[dict]:
    async with _sync_lock:
        return await asyncio.to_thread(sync_configured_backend, maiconfig)


async def _sync_loop() -> None:
    interval = max(
        10, int(getattr(maiconfig, "maimaidx_storage_sync_interval_seconds", 60))
    )
    while True:
