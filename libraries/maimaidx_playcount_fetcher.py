...
from .maimaidx_qrcode_util import qrcode_log_preview
...
class PlayCountFetcher:
    ...
    async def login_by_sdgb(self, qrcode_data: str, qqid: int) -> bool:
        ...
        pc_db.save_credential(cred)
        log.info(
            f'[SwApi] 用户 {qqid} 二维码凭据已保存 qrcode={qrcode_log_preview(qrcode_data)}'
        )
        return True
    ...
    async def fetch_via_sdgb_with_retry(
        ...
        *,
        ...
        """通过 sw-api 拉取全量 PC 数据；HTTP 500 时等待后重试。"""
        cred = pc_db.get_credential(qqid)
        if cred is None:
            raise RuntimeError(f'用户 {qqid} 尚未登录，请先使用「更新pc数」命令')

        qr_text = cred.credential_data
        log.info(f'[SwApi] 用户 {qqid} 正在拉取全量成绩 qrcode={qrcode_log_preview(qr_text)}')
        try:
