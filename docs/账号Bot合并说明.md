...
- 跨平台 Koishi identity / V2 identity migration
...
源码目录 `/Users/wujian/Documents/maibot` 中未发现 SQLite、MySQL 导出或
Koishi 运行配置，因此本次没有可安全自动导入的运行数据。拿到数据库导出后，使用
`scripts/migrate_maibot_accounts.py` 导入账号、二维码、上传 Token 与用户协议记录。
可以直接传入包含多个 Koishi 插件表的完整 SQLite 数据库：迁移器以只读方式打开源库，
只读取 `maibot_bindings` 和可选的 `maibot_user_terms`，其它插件表不会读写或删除。
