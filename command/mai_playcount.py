...
from ..libraries.maimaidx_whitelist import (
    VERIFY_KEYWORD,
    add_to_whitelist,
    is_whitelisted,
    whitelist_db,
...
verify_cmd = on_command('verify', aliases={'Verify', 'VERIFY'})
...
def _check_whitelist(event: MessageEvent) -> bool:
    """检查用户是否在白名单中，如果不在则返回 False 并自动 finish。"""
    if not is_whitelisted(event.user_id):
        return False
    return True
...
async def handle_df_bind(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    if not _check_whitelist(event):
        await df_bind.finish(
            ...
            + MessageSegment.text(
                '\n此功能为白名单功能，请阅读\n'
                'https://blog.baka86.love/posts/bot-user-agreement/\n'
                '后按照提示前往官群 975995705 验证。'
            )
        )
...
@verify_cmd.handle()
async def handle_verify(event: MessageEvent, arg: Message = CommandArg()):
    """处理「/verify <token>」命令，校验后进入二次确认流程。"""
    token = arg.extract_plain_text().strip()
    qqid = event.user_id

    if token != 'baka86.love':
        return

    if not isinstance(event, GroupMessageEvent) or event.group_id != 975995705:
        await verify_cmd.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text('该指令仅限官群验证使用，请前往 Bot 官群 975995705 验证。')
        )

    if is_whitelisted(qqid):
        await verify_cmd.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text('你已经在白名单中。')
        )

    _pending_verify[qqid] = True
    await verify_cmd.send(
        MessageSegment.reply(event.message_id)
        + MessageSegment.text(
            '\n验证完毕后即代表您同意了我们的使用协议\n'
            '确认验证吗？确认请发「是」'
...
@verify_cmd.receive()
async def receive_verify_confirm(event: GroupMessageEvent):
    """接收用户的二次确认回复。"""
    qqid = event.user_id

    if qqid not in _pending_verify:
        return

    answer = event.get_plaintext().strip()
    if answer != '是':
        del _pending_verify[qqid]
        await verify_cmd.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text('已取消验证。')
        )

    del _pending_verify[qqid]

    success = add_to_whitelist(qqid, note='verify')
    if success:
        await verify_cmd.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text(f'✅ 验证成功，已加入白名单。\n当前白名单人数: {whitelist_db.count()}')
        )
    else:
        await verify_cmd.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text('加入白名单失败，请稍后重试。')
        )
...
async def handle_query_charge(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    if not is_whitelisted(qqid):
        await query_charge.finish(
            MessageSegment.reply(event.message_id)
            + MessageSegment.text(
                '\n此功能为白名单功能，使用即代表同意用户协议，请阅读\n'
                'https://blog.baka86.love/posts/bot-user-agreement/\n'
                '后按照提示前往官群 975995705 验证。'
            )
        )
